      ...
      <view style="min-height: 50px;">
        ...
        >
          <view class="zb-cell__title"
                :class="[{
                  'active-cell__title':value===item.value
                }]"
                :style="[{
                  color:value===item.value?activeColor:''
                }]"
          >
            <text>{{item.text}}</text>
          </view>
          <view class="zb-cell__value" v-if="value===item.value">
            <text
  ...
  methods:{
    ...
    bindRelation() {
      let obj = this.parent.childrenList.find(item=>{
        let flag = this.options.some(ite=>item.title===ite.text)
